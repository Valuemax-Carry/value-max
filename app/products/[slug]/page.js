...
export default function Page({ params }) {
  ...
  const filtered = selectedCompany === "All" ? products : products.filter((p) => p.company === selectedCompany);
  ...
  return (
    <>
      <style>{`
        ...
        .vm-card { transition: transform 0.22s cubic-bezier(0.22,1,0.36,1), box-shadow 0.22s ease; }
        .vm-card:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(182,10,1,0.1); }
          ...
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div className="vm-sidebar absolute right-0 top-0 bottom-0 w-72 bg-white shadow-2xl flex flex-col">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <span className="font-bold text-gray-900 text-sm">Filter by Brand</span>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-1.5">
                {companies.map((c) => (
                  <button
                    ...
                    className={`vm-company-btn w-full text-left px-4 py-2.5 rounded-xl text-sm font-medium ${
                      selectedCompany === c
                        ? "bg-[#b60a01] text-white shadow-sm"
                        : "text-gray-600 hover:bg-red-50 hover:text-[#b60a01]"
                    }`}
                  >
                    {c}
                    {c === "All" && <span className={`ml-2 text-[11px] font-semibold ${selectedCompany === c ? "text-red-200" : "text-gray-400"}`}>{products.length}</span>}
                    {c !== "All" && (
                      <span className={`ml-2 text-[11px] font-semibold ${selectedCompany === c ? "text-red-200" : "text-gray-400"}`}>
                        {products.filter((p) => p.company === c).length}
                      </span>
                    )}
                  </button>
                ))}
              ...
              <div className="p-3 space-y-1">
                {companies.map((c) => (
                  <button
                    ...
                    className={`vm-company-btn w-full text-left px-3.5 py-2.5 rounded-xl text-[13px] font-medium flex items-center justify-between ${
                      selectedCompany === c
                        ? "bg-[#b60a01] text-white"
                        : "text-gray-600 hover:bg-red-50 hover:text-[#b60a01]"
                    }`}
                  >
                  ...
                  >
                    ...
                    <div className="px-4 py-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button className="bg-[#b60a01] hover:bg-[#9a0800] text-white text-[11px] font-bold px-3.5 py-1.5 rounded-lg transition-colors duration-150">
                          Add to Cart
                        </button>
                      </div>
                    </div>
