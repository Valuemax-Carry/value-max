...
export default function Page({ params }) {
  // params may be undefined or slug may be an array; guard defensively
  const rawSlug = params?.slug ?? "";
  const slug = Array.isArray(rawSlug) ? rawSlug[0] : rawSlug;
  const products = slug ? getProductsBySlug(slug) : [];
  const title = slug ? slug.replace(/-/g, ' ') : 'All Products';

  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
      <h1 className="text-3xl font-extrabold mb-6">Products: {title}</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {products.length ? (
          ...
          <div className="col-span-full text-center text-gray-500 py-12">No products found for this category.</div>
        )}
      </div>
    </div>
  );
